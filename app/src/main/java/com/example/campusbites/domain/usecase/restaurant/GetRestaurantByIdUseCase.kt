...
class GetRestaurantByIdUseCase @Inject constructor(
    ...
    suspend operator fun invoke(id: String): RestaurantDomain? {
        val restaurantDTO = repository.getRestaurantById(id) ?: return null
        return RestaurantDomain(
            ...
            foodTags = restaurantDTO.foodTagsIds.map { foodTagId -> getFoodTagByIdUseCase(foodTagId) },
            dietaryTags = restaurantDTO.dietaryTagsIds.map { dietaryTagId -> getDietaryTagByIdUseCase(dietaryTagId) },
